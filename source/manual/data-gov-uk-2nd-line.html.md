...
If there's not already an issue logged, considering opening a new one or - if you know how to fix it - open a PR to resolve the problem.
...
Traffic for data.gov.uk is recorded using Google Analytics, in specific properties. If a user requests analytics for data.gov.uk, we can provide them with access to [an analytics dashboard][dgu-analytics].
