...
* **[`unprocessed content changes`](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/process_content_change_job.rb)**.
...
* **[`unprocessed messages`](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/process_message_job.rb)**.
...
* **`incomplete digest runs`**.
  ...
  * This could be due to a failure in any of three workers:
    ...
    * [\[Daily/Weekly\]DigestInitiatorJob](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/daily_digest_initiator_job.rb) generates a DigestRunSubscriber work item for each subscriber.
    * [DigestEmailGenerationJob](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/digest_email_generation_job.rb) does the work of generating the digest email for a specific subscriber.
    * [DigestRunCompletionMarkerJob](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/digest_run_completion_marker_job.rb) periodically scans all the work items to see if the run is complete.

Each of the alerts is based on custom metrics that we collect using [a periodic job](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/metrics_collection_job.rb). The metric will be something like "amount of unprocessed work older than X amount of time" ([example](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/metrics_collection_job/content_change_exporter.rb#L16)).

## Automatic recovery

Sometimes we lose work due to [a flaw with the Sidekiq queueing system](https://github.com/mperham/sidekiq/wiki/Problems-and-Troubleshooting#my-sidekiq-process-is-crashing-what-do-i-do). In order to cope with this scenario, a [RecoverLostJobsJob] runs every 30 minutes, and will try to requeue work that has not been processed [within an hour](https://github.com/alphagov/email-alert-api/blob/main/app/jobs/recover_lost_jobs_job/unprocessed_check.rb#L13). If work is being repeatedly lost, the alert will fire and you'll need to investigate manually.
...
> DigestRunCompletionMarkerJob.new.perform
> ```
...
[RecoverLostJobsJob]: https://github.com/alphagov/email-alert-api/blob/main/app/jobs/recover_lost_jobs_job.rb
