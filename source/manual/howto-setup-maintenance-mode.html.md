...
During planned outages, E2E (end-to-end) tests — if any — should be disabled to prevent CI/CD pipelines from failing or blocking deployments while the application is unavailable like e.g. [govuk-e2e-tests](https://github.com/alphagov/govuk-e2e-tests/pull/348) and re-enabled once the system downtime is completed.
...
3. Scale down the application and/or workers by setting `appEnabled: false` as in [here](https://github.com/alphagov/govuk-helm-charts/pull/3672/commits/d377bdb7379c303e2f5c6d52ad1dcecdc1babbe8). This helps prevent job processing while it's in maintenance mode.
