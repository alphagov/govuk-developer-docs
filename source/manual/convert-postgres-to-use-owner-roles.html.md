...
* Make the existing app user and `aws_db_admin` users members of the new owner role
* Grant the new owner role permissions to the database objects
* Reassign ownership for the database and its objects to the new owner role
* Repeat the above process for Staging and Integration
* Update the `db-backup` Helm Chart to use the new role with the `DB_OWNER` environment variable
...
Running `\l` should return a table of roles that looks like this:
...
Running `\du` should return a table of roles that looks like this:
