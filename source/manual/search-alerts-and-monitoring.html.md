...
We also have alerting on search result quality. Currently there are four additional Alertmanager rules related to search result quality configured in govuk-helm-charts to notify the #govuk-search-alerts channel.
...
- SearchQualityDegradedBinaryRecallDrop Top 3 recall for binary query set has dropped below 90%.

- SearchQualityDegradedBinaryRecallCritical Top 3 recall for binary query set has dropped below 80%.

- SearchQualityDegradedClickstreamNDCGDrop Top 10 NDCG for clickstream query set has dropped below 85%.

- SearchQualityDegradedClickstreamNDCGCritical Top 10 NDCG for clickstream query set has dropped below 75%.
