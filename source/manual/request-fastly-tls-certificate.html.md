...
1. At this point, a unique domain [ownership validation](https://docs.fastly.com/en/guides/serving-https-traffic-using-fastly-managed-certificates#verifying-domain-ownership) record (`_acme-challenge`) is generated by Fastly.
   > If you see the error `Found conflicting CAA record(s). Expected globalsign.com` alongside the domain, this means the domain owner has restricted which providers can issue certificates for the domain. Ask the domain owner to add `globalsign.com` as a CAA record for the domain.
