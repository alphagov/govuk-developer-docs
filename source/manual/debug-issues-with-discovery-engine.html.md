...
title: "Debug issues with Discovery Engine"
parent: "/manual.html"
...
[link-3]: https://github.com/alphagov/search-admin/blob/main/lib/tasks/document.rake#L15
[link-4]: https://docs.cloud.google.com/generative-ai-app-builder/docs/reference/rest/v1/projects.locations.collections.dataStores.branches.documents/get
[link-5]: https://docs.cloud.google.com/generative-ai-app-builder/docs/reference/rest/v1/projects.locations.collections.dataStores/completeQuery
...
> [!IMPORTANT]
> Remember to revoke any local auth credentials once you’ve finished working with gcloud

To revoke local auth credentials:
...
A [rake task][link-3] exists to provide a convenient way for a developer to quickly check if a document is present in the data store. The rake task pretty prints the `json_data` of the document.
...
You can get a document’s JSON representation from the datastore using the [projects.locations.collections.dataStores.branches.documents.get][link-4] endpoint (replace `<content_id>` at the end with the UUID-format GOV.UK content ID):
...
To check that autocomplete is working correctly you can query the [projects.locations.collections.dataStores.completeQuery][link-5] endpoint (replace `<query term>` with your query string):

```
curl -sH "Authorization: Bearer $(gcloud auth print-access-token)" \
-H "Content-Type: application/json" \
-H "X-Goog-User-Project: search-api-v2-production" \
"https://discoveryengine.googleapis.com/v1/projects/search-api-v2-production/locations/global/collections/default_collection/dataStores/govuk_content/:completeQuery?query=<query term>&queryModel=user-event&includeTailSuggestions=false"
```

### Example responses

> [!TIP]
> Autocomplete suggestions are based on popular recent searches. It is therefore possible for autocomplete to return no suggestions for a query without it being broken.
> Some common queries that we __do__ expect responses for include "universal", "tax", "income".

#### Autocomplete is working

Example response for query term `proof`:

```json
{
    "querySuggestions": [
        {
            "suggestion": "proof of benefit"
        },
        {
            "suggestion": "proof of benefit letter"
        },
        {
            "suggestion": "proof of pip"
        },
        {
            "suggestion": "proof of immigration status"
        },
        {
            "suggestion": "proof of right to work"
        }
    ]
}
```
...
If autocomplete is not working, it is likely to return an empty array, which looks like this:
